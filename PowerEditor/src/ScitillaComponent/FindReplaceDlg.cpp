	...
	if (posFind == -1) //no match found in target, check if a new target should be used
	{
		...
		if (posFind == -1)
		{
			...
			{	
				...
				// if the dialog is not shown, pass the focus to his parent(ie. OurNotepad)
...
void Finder::copy()
{
	...
	if (!toClipboard.empty())
	{
		...
		{
			assert(false);
			::MessageBox(NULL, TEXT("Error placing text in clipboard."), TEXT("OurNotepad"), MB_ICONINFORMATION);
		}
	}
}
