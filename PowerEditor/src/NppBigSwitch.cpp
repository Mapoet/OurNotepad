...
LRESULT Notepad_plus::process(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		...
		case NPPM_DESTROYSCINTILLAHANDLE:
		{
			...
			// Destroying allocated Scintilla makes OurNotepad crash
			...
			// So we do nothing here and let OurNotepad destroy allocated Scintilla while it exits
		...
		case WM_CLOSE:
		{
			...
			else
			{
				...
				//Sends WM_DESTROY, OurNotepad will end
				if (message == WM_CLOSE)
					::DestroyWindow(hwnd);
            ...
            // then WM_ENDSESSION is sent with wParam == FALSE - OurNotepad should exit in this case
