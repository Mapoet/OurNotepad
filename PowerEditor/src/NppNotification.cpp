		...
		case TCN_TABDROPPED:
		{
			...
			if (notification->nmhdr.code == TCN_TABDROPPEDOUTSIDE)
			{
				...
				else
				{
					...
					if (lstrcmp(className, _pPublicInterface->getClassName()) == 0 && hWinParent != _pPublicInterface->getHSelf()) // another OurNotepad
					{
						...
						if (buf->isDirty())
						{
							...
							_nativeLangSpeaker.messageBox("CannotMoveDoc",
								...
								TEXT("Move to new OurNotepad Instance"),
								MB_OK);
						}
						else
						{
					...
					else // Not OurNotepad, we open it here
					{
