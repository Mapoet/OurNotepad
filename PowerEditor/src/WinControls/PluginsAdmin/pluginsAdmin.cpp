...
bool PluginsAdminDlg::exitToInstallRemovePlugins(Operation op, const vector<PluginUpdateInfo*>& puis)
{
	...
	auto res = pNativeSpeaker->messageBox("ExitToUpdatePlugins",
		_hSelf,
		TEXT("If you click YES, you will quit OurNotepad to continue the operations.\nOurNotepad will be restarted after all the operations are terminated.\nContinue?"),
		TEXT("OurNotepad is about to exit"),
	...
	if (res == IDYES)
	{
		...
		// application path: OurNotepad path to be relaunched
		// plugin global path
		...
		// Quite OurNotepad so just before quitting OurNotepad launches gup with needed arguments  
...
bool PluginsAdminDlg::isValide()
{
	...
	// check the signature on default location : %APPDATA%\OurNotepad\plugins\config\pl\nppPluginList.dll or NPP_INST_DIR\plugins\config\pl\nppPluginList.dll
