...
bool Notepad_plus::doReload(BufferID id, bool alert)
{
	if (alert)
	{
		int answer = _nativeLangSpeaker.messageBox("DocReloadWarning",
			_pPublicInterface->getHSelf(),
			TEXT("Are you sure you want to reload the current file and lose the changes made in OurNotepad?"),
			TEXT("Reload"),
...
bool Notepad_plus::doSave(BufferID id, const TCHAR * filename, bool isCopy)
{
	...
	if (!res)
	{
		// try to open OurNotepad in admin mode
		if (!_isAdministrator)
		{
			bool isSnapshotMode = NppParameters::getInstance().getNppGUI().isSnapshotMode();
			if (isSnapshotMode) // if both rememberSession && backup mode are enabled
			{                   // Open the 2nd OurNotepad instance in Admin mode, then close the 1st instance.
				int openInAdminModeRes = _nativeLangSpeaker.messageBox("OpenInAdminMode",
				_pPublicInterface->getHSelf(),
				TEXT("The file cannot be saved and it may be protected.\rDo you want to launch OurNotepad in Administrator mode?"),
				TEXT("Save failed"),
				MB_YESNO);
				...
				if (openInAdminModeRes == IDYES)
				{
					...
					if (shellExecRes < 32)
					{
						_nativeLangSpeaker.messageBox("OpenInAdminModeFailed",
							_pPublicInterface->getHSelf(),
							TEXT("OurNotepad cannot be opened in Administrator mode."),
			...
			{    // open only the file to save in OurNotepad of Administrator mode by keeping the current instance.
	...
	if (pfn)
	{
		...
		else		//cannot save, other view has buffer already open, activate it
		{
			_nativeLangSpeaker.messageBox("FileAlreadyOpenedInNpp",
				_pPublicInterface->getHSelf(),
				TEXT("The file is already opened in OurNotepad."),
				TEXT("ERROR"),
				MB_OK | MB_ICONSTOP);
			switchToFile(other);
			return false;
