...
private:
	...
	// a handle list of all the OurNotepad dialogs
	...
	bool _isEndingSessionButNotReady = false; // If Windows 10 update needs to restart 
                                              // and OurNotepad has one (some) dirty document(s)
                                              // and "Enable session snapshot and periodic backup" is not enabled
                                              // then WM_ENDSESSION is send with wParam == FALSE
                                              // in this case this boolean is set true, so OurNotepad will quit and its current session will be saved 
