...
const TCHAR * USERMSG = TEXT(" is not compatible with the current version of OurNotepad.\n\n\
...
int PluginsManager::loadPlugin(const TCHAR *pluginFilePath)
{
	...
	try
	{
		...
		if (!pi->_pFuncIsUnicode || !pi->_pFuncIsUnicode())
			throw generic_string(TEXT("This ANSI plugin is not compatible with your Unicode OurNotepad."));
