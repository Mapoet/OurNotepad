	...
	switch (id)
	{
		...
		case IDM_EDIT_SEARCHONINTERNET:
		{
			...
			if (nppGui._searchEngineChoice == nppGui.se_custom)
			{
				...
				auto temp=remove_if(url.begin(), url.end(), _istspace);
		...
		case IDM_SETTING_IMPORTPLUGIN :
        {
            ...
            const TCHAR *extFilterName = TEXT("OurNotepad plugin");
            const TCHAR *extFilter = TEXT(".dll");
            vector<generic_string> copiedFiles = addNppPlugins(extFilterName, extFilter);

            // Tell users to restart OurNotepad to load plugin
			if (copiedFiles.size())
			{
				NativeLangSpeaker *pNativeSpeaker = (NppParameters::getInstance()).getNativeLangSpeaker();
				pNativeSpeaker->messageBox("NeedToRestartToLoadPlugins",
					NULL,
					TEXT("You have to restart OurNotepad to load plugins you installed."),
					TEXT("OurNotepad need to be relaunched"),
					MB_OK | MB_APPLMODAL);
			}
            break;
        }
        ...
        case IDM_SETTING_IMPORTSTYLETHEMS :
        {
            ...
            const TCHAR *extFilterName = TEXT("OurNotepad style theme");
        ...
        case IDM_SETTING_EDITCONTEXTMENU :
        {
			_nativeLangSpeaker.messageBox("ContextMenuXmlEditWarning",
				_pPublicInterface->getHSelf(),
				TEXT("Editing contextMenu.xml allows you to modify your OurNotepad popup context menu on edit zone.\rYou have to restart your OurNotepad to take effect after modifying contextMenu.xml."),
		...
		case IDM_HELP :
		{
			...
			else
			{
				...
				if (!_nativeLangSpeaker.getMsgBoxLang("NppHelpAbsentWarning", title, warning))
				{
					...
					warning = TEXT("\rdoesn't exist. Please download it on OurNotepad site.");
				}
				msg += warning;
		...
		{
			// No translattable
			::MessageBox(_pPublicInterface->getHSelf(), COMMAND_ARG_HELP, TEXT("OurNotepad Command Argument Help"), MB_OK | MB_APPLMODAL);
			break;
		}
		...
		case IDM_CONFUPDATERPROXY :
		{
			...
			if (ver <= WV_XP)
			{
				long res = _nativeLangSpeaker.messageBox("XpUpdaterProblem",
					_pPublicInterface->getHSelf(),
					TEXT("OurNotepad updater is not compatible with XP due to the obsolete security layer under XP.\rDo you want to go to OurNotepad page to download the latest version?"),
					TEXT("OurNotepad Updater"),
					MB_YESNO);
					...
					if (id == IDM_CONFUPDATERPROXY)
					{
						if (!_isAdministrator)
						{
							_nativeLangSpeaker.messageBox("GUpProxyConfNeedAdminMode",
								_pPublicInterface->getHSelf(),
								TEXT("Please relaunch OurNotepad in Admin mode to configure proxy."),
								TEXT("Proxy Settings"),
								MB_OK | MB_APPLMODAL);
							return;
						}
